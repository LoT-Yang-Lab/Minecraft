/**
 * 九石阵：与 GameCrafting 订单逻辑一致；键位与 3×3 网格导航统一（见 NCGridGeometry）。
 */
(function (global) {
  var gm = global.NCGridGeometry;
  if (!gm || !gm.neighbor) {
    throw new Error("crafting_runner.js 需要先加载 grid_geometry.js（NCGridGeometry）");
  }
  var applyChain =
  ...
  function nextState(_rules, state, keyLower) {
    var spec = KEY_TO_EDGE[keyLower];
    if (!spec) return { ok: false, next: null, reason: "bad_key" };
    var code = stoneToCode(normalizeStoneId(state));
    var nxtCode = gm.neighbor(code, keyLower);
    if (!nxtCode) return { ok: false, next: null, reason: "no_edge" };
    return {
      ok: true,
      next: "stone_" + String(nxtCode).padStart(2, "0"),
      reason: "ok",
    };
  }
  ...
  /** 与导航 start_code/goal_code（1–9）对齐：石块 id → 整数编码 */
  function stoneToCode(sid) {
    return stoneIndex(normalizeStoneId(sid));
  }

  /** 与导航 steps.direction 格式一致：药水槽 + 几何方向（中文） */
  var DIR_ZH = { left: "左", right: "右", up: "上", down: "下", cw: "角点顺时针" };

  function craftDirectionDisplay(keyLower) {
    var dirEn = gm.KEY_DIR[keyLower];
    var zh = DIR_ZH[dirEn] || String(dirEn || "");
    var slot = keyToPotionSlot(keyLower);
    var pot =
      slot === 1 ? "药水1" : slot === 2 ? "药水2" : slot === 3 ? "药水3" : "药水";
    var ku = keyLower.toUpperCase();
    return pot + "(" + ku + ")·" + zh;
  }

  function orderTargetsToCodes(arr) {
    if (!Array.isArray(arr)) return [];
    var out = [];
    for (var i = 0; i < arr.length; i++) {
      var c = stoneToCode(arr[i]);
  ...
  function runCraftingTrial(display_element, rules, trial, phase, trialIndex, done, progressOpt) {
    ...
    function finishTrial() {
      ...
      var gCodes = orderTargetsToCodes(targets);
      var gCode = gCodes.length ? gCodes[gCodes.length - 1] : 0;
      var tid =
        trial.trial_id != null && String(trial.trial_id).length
          ? String(trial.trial_id)
          : trial.task_id != null && String(trial.task_id).length
            ? String(trial.task_id)
            : "";
      done({
        phase: phase,
        domain: "crafting",
        ...
        trial_progress:
          ...
          typeof progressOpt.total === "number"
            ...
            : null,
        trial_id: tid,
        start_code: stoneToCode(startState),
        goal_code: gCode,
        goal_codes: gCodes,
        steps: steps,
    ...
    function onKey(ev) {
      ...
      if (!mv.ok || !mv.next) {
        ...
        steps.push({
          ...
          key: k,
          valid: false,
          from_code: stoneToCode(state),
          to_code: stoneToCode(state),
          mode: "grid",
          direction: craftDirectionDisplay(k),
          dir_code: gm.KEY_DIR[k],
          order_target: target ? stoneToCode(target) : null,
